...
* `Thread` - In most configurations, every Java thread can use up to **1MB** (`-Xxx`). Increasing consumption in that category usually means:
...
When you run your Java code you are allowed to invoke native code with JNI (and other mechanisms, but that's irrelevant). The code inside the external native library is out of JVM's control.
...
Nowadays, I see that problem most commonly when engineers decide to use an _Amazon Linux_, usually by fetching an _Amazon Corretto_ docker image:
