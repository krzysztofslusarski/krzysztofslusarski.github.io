...
| ```int``` lanes per vector | **4** | **16** | **8** |
| Vector ```int``` ```add``` latency | **2 cycles** | **1 cycle** | **1 cycle** |

Specs for that table:

- **Graviton5** is built on the [Arm Neoverse V3](https://developer.arm.com/documentation/109678/latest/){:target="_blank"}
  core. Its optimization guide states an SVE implementation with a **128-bit vector length**, and gives both
  ```ASIMD arith, basic``` and SVE ```Arithmetic, basic``` - the groups that contain ```ADD``` - a latency of **2** and
  a throughput of **4**. **Graviton4** ([Neoverse V2](https://developer.arm.com/documentation/109898/latest/){:target="_blank"})
  has exactly the same numbers, so everything written above about accumulators applies to both.
- **Xeon 6** ```vpaddd``` on 512-bit registers: [uops.info](https://uops.info/html-instr/VPADDD_ZMM_ZMM_ZMM.html){:target="_blank"}.
  That table does not cover ```Granite Rapids``` yet, but the latency of **1** holds for every **AVX-512** generation it
  does cover, from ```Skylake-X``` up to ```Emerald Rapids```.
- Mind the split inside the **Xeon 6** family - only the **P-cores** support **AVX-512**. The **E-cores** top out at
  **AVX2**, so on the very same product line you get **16** lanes or **8**, depending on the model you rent.
