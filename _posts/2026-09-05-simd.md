...
This example is not synthetic at all, although very similar o previous one. 
Some time ago I had to speed up an algorithm whose first part had to count
negative numbers in an array. The scalar code is simple:
...
remove the branch and vectorize the loop at the same time. The branch removal happens too late, when the vectorizer is
already done with this loop. So we need to get rid of the branch ourselves and figure out a branchless version of this
...
generate all the **Vector API** code, to find proper CPU specs and better description of vectors in the first chapter.